...
object EmailTools {
    ...
    fun checkEmailTool(emailStore: EmailStore) = object : Tool {
        override val schema = ToolSchema(
            name = "check_email",
            description = "List emails that have arrived since the last time Mr. Robot surfaced new mail to the user. Mr. Robot tracks delivery internally and ignores the provider's read flag, so an email shows up here at most once whether it was first seen via heartbeat or a previous check_email. To find an email that's already been surfaced (or any older message), use search_email with `from` / `subject` / `since`. If multiple accounts are connected, checks all of them.",
            parameters = mapOf(
        ...
        override suspend fun execute(args: Map<String, Any>): Any {
            ...
            return buildMap {
                ...
                if (allEmails.isEmpty()) {
                    put(
                        "hint",
                        "No new emails since the last delivery. To find a message Mr. Robot has already surfaced (or any older email), call search_email with the account_id from `accounts` and a `from` / `subject` / `since` filter.",
                    )
                }
            }
        }
