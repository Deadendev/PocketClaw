...
enum class SystemPromptVariant {
     ...
     * memory category dumps, scheduled tasks, Structured Learning guidance, and pocketclaw-ui
...
/** Which pocketclaw-ui section, if any, to render. */
 ...
 * admitting a tool returned nothing readable, and (2) pocketclaw-ui buttons whose labels
...
private fun StringBuilder.appendDynamicUiSection() {
    append("\n## Dynamic UI\n")
    append("You can enhance your chat responses with interactive UI elements using pocketclaw-ui blocks. ")
    ...
    append("Use pocketclaw-ui whenever collecting data, offering choices, presenting structured information, or guiding multi-step workflows. ")
    append("You can mix pocketclaw-ui blocks with regular markdown text naturally — use markdown for explanations and pocketclaw-ui for interactive elements.\n\n")
    append(POCKETCLAW_UI_COMPONENT_CATALOG)
    append("Layout tips:\n")
...
private fun StringBuilder.appendInteractiveUiSection() {
    ...
    append("You are in full-screen interactive UI mode. The user ONLY sees rendered pocketclaw-ui components — they cannot see markdown, plain text, or anything outside a pocketclaw-ui fence.\n")
    append("Your ENTIRE response must be a single ```pocketclaw-ui code fence. No text before it, no text after it, no markdown. If you write anything outside the fence, the user will NOT see it.\n\n")
    append(POCKETCLAW_UI_COMPONENT_CATALOG)
    append("Rules:\n")
    append("- Each response is a COMPLETE screen layout. Include all content and actions in one pocketclaw-ui block.\n")
...
/**
 * Pre-computed pocketclaw-ui component catalog text — shared between [appendDynamicUiSection]
 ...
 */
private val POCKETCLAW_UI_COMPONENT_CATALOG: String = buildString {
    append("Format: wrap a JSON object in ```pocketclaw-ui fences.\n\n")
