...
import com.inspiredandroid.pocketclaw.sendHeartbeatNotification
    ...
    private suspend fun runHeartbeat() {
        ...
        try {
            ...
            if (response.isNotBlank() && "HEARTBEAT_OK" !in response) {
                ...
                // Strip markdown + pocketclaw-ui fences before sending to the tray —
                // the notification surface can't render them and raw fence
                // text (```pocketclaw-ui {...}```) is unreadable.
                if (!appInForeground) {
