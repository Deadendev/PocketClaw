...
import com.inspiredandroid.pocketclaw.ui.markdown.toSpeakableText

/**
 * TTS-friendly text for a message that may contain pocketclaw-ui fences. Routed through the unified
 * markdown parser: formatting is stripped, code blocks dropped, and pocketclaw-ui blocks walked for
...
internal fun PocketClawUiNode.collectSpeakableText(): String {
    val parts = mutableListOf<String>()
    walk(parts)
    return parts.asSequence().filter { it.isNotBlank() }.joinToString(". ")
}

private fun PocketClawUiNode.walk(parts: MutableList<String>) {
    when (this) {
