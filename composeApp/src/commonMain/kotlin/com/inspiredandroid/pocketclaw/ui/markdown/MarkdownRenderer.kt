...
import com.inspiredandroid.pocketclaw.ui.markdown.math.MathFormula
 ...
 *
 * Kai-UI blocks dispatch to [PocketClawUiRenderer]; pass `isInteractive = false` to render them as
...
fun MarkdownContent(
    ...
    val doc = remember(content) {
        runCatching { parseMarkdown(content) }.getOrElse {
            MarkdownDocument(persistentListOf(Paragraph(persistentListOf(com.inspiredandroid.pocketclaw.ui.markdown.Text(content)))))
        }
    }
...
private fun BlockRenderer(
    ...
    when (block) {
        ...
        is PocketClawUiBlock -> PocketClawUiRenderer(
            node = block.node,
        ...
        is PocketClawUiError -> CodeFenceBlock(
            language = "json",
            code = block.rawJson,
            modifier = Modifier.padding(vertical = 4.dp),
        )
