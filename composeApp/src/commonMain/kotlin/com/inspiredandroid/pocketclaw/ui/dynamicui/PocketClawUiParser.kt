...
/**
 * Decodes the body of a `pocketclaw-ui` fenced JSON block into a [PocketClawUiNode].
 *
 * The parse pipeline runs each block through three stages:
 *   1. **Block extraction** — the markdown parser locates `pocketclaw-ui` fences and hands the raw
 ...
 *      [parseNode] in `PocketClawUiNodeBuilders.kt`, constructing [PocketClawUiNode] instances field-by-field.
 ...
 */
object PocketClawUiParser {

    // =========================================================================================
    // Public API
    // =========================================================================================

    /** Result of decoding a pocketclaw-ui fence body; consumed by the markdown parser. */
    sealed interface UiBlockResult {
        data class Ui(val node: PocketClawUiNode, val rawJson: String) : UiBlockResult
        data class Error(val rawJson: String) : UiBlockResult
    }

    /**
     * Decode the raw body of a pocketclaw-ui fence (everything between the opening and closing triple
     * backticks). Returns either a decoded [PocketClawUiNode] or an [UiBlockResult.Error] carrying the
    ...
    fun parseUiBlockBody(rawBlock: String): UiBlockResult? {
        ...
        return try {
            ...
            println("pocketclaw-ui parse error: ${e.message} | ${json.take(500)}")
    ...
    private fun tryParseLine(line: String): PocketClawUiNode? = runCatching { parseSingleNode(line) }.getOrNull()
        ?: runCatching { parseSingleNode(sanitizeJson(line)) }.getOrNull()
        ?: run {
            println("pocketclaw-ui parse error: failed to deserialize line | ${line.take(500)}")
            null
        }

    /** Parse a repaired JSON string into a [PocketClawUiNode] via the direct builder pipeline. */
    private fun parseSingleNode(json: String): PocketClawUiNode? = parseNode(SharedJson.parseToJsonElement(json))
